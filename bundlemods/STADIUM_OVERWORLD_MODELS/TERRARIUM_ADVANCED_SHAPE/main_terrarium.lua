-- ------- combo-package note
--
-- This file is TERRARIUM's own main.lua, unmodified apart from three
-- purely mechanical changes so it can share a package with ADVANCED_SHAPE
-- (formerly upstream DRAMATIC_SHAPE) without their files colliding on
-- disk:
--
--   1. chunkFor/V.data below read from lib_terrarium/ and data_terrarium/
--      instead of lib/ and data/, since ADVANCED_SHAPE ships its OWN,
--      differently-written files under those same names (this pair of
--      forks diverged well past main.lua -- Water.lua, Voxel3D.lua and
--      several others are genuinely different modules per fork, not a
--      renamed constant apart) and only one file can sit at a given path.
--   2. Two guard flags this file sets on shared ENGINE classes --
--      Map.terrarium_blockHook and OptionsMenu.terrarium_fullHook --
--      are renamed to terrarium_blockHook / terrarium_fullHook. Both forks
--      inherited the same flag NAME from their common ancestor; sharing a
--      package now means whichever mod's main.lua ran first would set the
--      flag and the second would read it already true, silently skipping
--      its own hook install. Not a risk when each fork is its own separate
--      mod install -- only once both run in the same Lua state.
--   3. mod.exports.version/lib/pipelines/keys, at the bottom, write into a
--      namespaced mod.exports.terrarium table instead of the flat fields,
--      for the same reason: ADVANCED_SHAPE's main.lua sets those same four
--      flat fields, and the two would otherwise overwrite each other
--      depending purely on load order.
--
...
function V.require(name)
  ...
  -- combo package: TERRARIUM's own lib/, kept apart from ADVANCED_SHAPE's
  -- (see the note at the top of this file)
  local value = chunkFor("lib_terrarium/" .. name .. ".lua")(V)
  modules[name] = value
  return value
end
...
function V.data(name)
  ...
  if hit ~= nil then return hit end
  -- combo package: TERRARIUM's own data/
  local value = chunkFor("data_terrarium/" .. name .. ".lua")(V)
...
do
  ...
  if not Map.terrarium_blockHook then
...
do
  ...
  if not OptionsMenu.terrarium_fullHook then
    local Pipelines = require("src.render.Pipelines")
    ...
    OptionsMenu.terrarium_fullHook = true
  end
end
...
-- pipelines/keys, since ADVANCED_SHAPE's own main.lua sets those same four
-- fields and the two would otherwise clobber each other by load order (see
-- the note at the top of this file). A companion mod that used to read the
-- flat fields for TERRARIUM now reads mod.exports.terrarium.* instead.
mod.exports.terrarium = {
  version = "1.15.0-mobile.snow.1",
  lib = V,
  pipelines = { voxel = PIPE_VOXEL, tiltshift = PIPE_TILT },
  keys = V.KEYS,
}
