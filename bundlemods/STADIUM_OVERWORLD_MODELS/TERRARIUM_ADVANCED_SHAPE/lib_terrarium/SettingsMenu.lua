...
SettingsMenu.ROOT_LABEL = "TERRARIUM.."
...
local function isFull()
  ...
  local pipe = (V and V.PIPE_VOXEL) or "voxel"
  return V.require("VoxelState").isFull(Pipelines.level(pipe))
end
...
function SettingsMenu.rows(catId, game)
  ...
  if catId == SettingsMenu.ROOT then
    for _, row in ipairs(pipelineRows) do
      ...
      local tiltId = "pipeline:" .. ((V and V.PIPE_TILT) or "tiltshift")
      if not (full and row.id == tiltId) then
        out[#out + 1] = row
      end
    end
