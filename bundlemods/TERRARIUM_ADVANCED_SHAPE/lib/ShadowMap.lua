...
-- Return the shadow softness value (edge blur width in texels)
function ShadowMap.softness()
  return 1.5  -- Default softness value
end
...
function ShadowMap.draw(mesh, texture, model)
  -- Wrap entire function in pcall to prevent errors from propagating
  return pcall(function()
    if not (drawing and mesh) then return end
    -- Skip if mesh is a table (not a drawable) - can happen in battle contexts
    if type(mesh) == "table" then return end
    local sh = getShader()
    if texture then pcall(function() if mesh.setTexture then mesh:setTexture(texture) end end) end
    pcall(sh.send, sh, "model", "row", model or IDENTITY)
    pcall(love.graphics.draw, mesh)
  end)
end

-- Draw a group of meshes with optional bounds (box parameter ignored for now)
function ShadowMap.drawGroup(group, texture, model, box)
  -- Wrap entire function in pcall to prevent errors from propagating
  return pcall(function()
    if not (drawing and group) then return end
    local sh = getShader()
    if not sh then return end
    pcall(sh.send, sh, "model", "row", model or IDENTITY)
    
    -- Handle both chunked groups and single meshes
    if group.chunks then
      local chunks = group.chunks
      for i = 1, #chunks do
        local ch = chunks[i]
        if not box or (ch.x1 >= box[1] and ch.x0 <= box[3]
                     and ch.z1 + ch.ymax >= box[2] and ch.z0 <= box[4]) then
          if ch.mesh and type(ch.mesh) ~= "table" then
            if texture and ch.mesh.setTexture then pcall(ch.mesh.setTexture, ch.mesh, texture) end
    ...
    elseif type(group) ~= "table" then
      -- Single mesh
      if texture and group.setTexture then pcall(group.setTexture, group, texture) end
      pcall(love.graphics.draw, group)
    end
  end)
end
