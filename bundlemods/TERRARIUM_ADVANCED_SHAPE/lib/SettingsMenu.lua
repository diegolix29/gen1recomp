...
-- Cosmetic only -- what the row itself says on screen. Left as
-- "ADVANCED SHAPE.." to match this mod's current branding (see main.lua's
-- header). The "DRAMATIC_SHAPE:" ID NAMESPACE just below is left
-- untouched: main.lua's own comments describe it as the settings rows'
-- identity "since the beginning", which reads as a persisted key prefix
-- rather than branding -- verify against manifest.json / ModSetting.lua
-- before renaming it, since a mismatch there silently breaks helpFor()
-- and the red-ink row lookup rather than erroring.
SettingsMenu.ROOT_LABEL = "ADVANCED SHAPE.."
...
SettingsMenu.CATEGORIES = {
  ...
  { id = "weather", label = "WEATHER..",
    help = "Wind, rain and snow, the ground they leave behind, and the "
      .. "birdsong and ambience under it all." },
  { id = "wildlife", label = "WILDLIFE..",
    help = "Wild Pokemon you can see standing in the grass, how many at "
      .. "once, and who is out at this hour." },
  { id = "town", label = "TOWN LIFE..",
    help = "Pokemon in the streets, asleep indoors, and the errands the "
      .. "map's own NPCs run on their own clock." },
  ...
  { id = "perf", label = "PERFORMANCE..",
    help = "What the look costs -- shadows, anti-aliasing, render "
      .. "resolution and draw distance, the most expensive things in the "
      .. "frame after the geometry itself." },
  { id = "fx", label = "VISUAL FX..",
    help = "Ray-traced light, cel shading and impact frames -- knobs on "
      .. "the LOOK rather than on what it costs." },
  { id = "qol", label = "QUALITY OF LIFE..",
    help = "Carrying more than six, auto-farming, shared experience, and "
      .. "the mercies that trim 1996's friction without touching the "
      .. "numbers." },
  { id = "vr", label = "VR..",
    ...
    for _, cat in ipairs(SettingsMenu.CATEGORIES) do
      ...
      if kids[1] then
        local countFunc = function()
          return ("%d SETTINGS"):format(#SettingsMenu.rows(cat.id, game))
        end
        -- A category's custom summary (BATTLES' and VR's own current rung)
        -- reads a setting this file does not own. Guarded so a mismatch
        -- against that setting's real API cannot crash the whole ROOT
        -- menu -- it falls back to the plain count instead, the same as a
        -- category with no custom summary at all.
        local custom = cat.summary
        local valueFunc = countFunc
        if custom then
          valueFunc = function()
