...
-- Whether the WEATHER has anything to paint at this instant. Asked alongside
-- the hour because the two want the same instant for the same reason: rain
-- belongs on the world, not on the dialog box in front of it, and this is the
-- one moment between the two (see the header). Guarded, because Weather
-- retires itself on a throw and a retired feature must cost the frame nothing.
local function weatherWants()
  local ok, wants = pcall(Weather.paintsFlat)
  return ok and wants or false
end
...
function DayTint.forFrame(renderer)
  ...
  if r > DayTint.NEUTRAL and g > DayTint.NEUTRAL and b > DayTint.NEUTRAL then
    -- A clear midday multiplies by white and is skipped, which is the whole
    -- reason a game with the clock at DAY issues not one extra call. But the
    -- WEATHER can want this instant even when the hour does not -- rain at
    -- noon is exactly that case -- so the frame is claimed at a neutral tint
    -- rather than skipped whenever there is something falling. Returning
    -- white is a multiply that changes no pixel: the rain rides in beside it
    -- and costs one rectangle nobody can see.
    if not weatherWants() then return nil end
    return 1, 1, 1
  end
  return r, g, b
end
...
function DayTint.paint(r, g, b)
  ...
  -- and the weather, over the hour rather than under it: the multiply is the
  -- LIGHT the world is standing in, and the rain is standing in that light
  -- too -- painting the streaks first and then darkening them would put the
  -- overcast sky in front of its own rain. Inside the same shader-off,
  -- scissor-off window the tint borrowed, which is exactly the state a
  -- screen-space pass wants and exactly the state this function already
  -- restores afterwards.
  pcall(Weather.paintFlat)
