...
-- MERGE NOTE: upstream DRAMATIC_SHAPE (Advanced Voxel Mod) and the
-- Terrarium fork are now ONE mod under the single id DRAMATIC_SHAPE, so the
-- old "give each fork its own registry keys so they can coexist" scheme is
-- gone -- there is only one voxel pipeline and one tiltshift pipeline now,
-- under their original upstream names, and other mods keyed to either the
-- old upstream pipeline name or this fork's should be pointed at these.
local PIPE_VOXEL = "voxel"
local PIPE_TILT  = "tiltshift"
V.PIPE_VOXEL, V.PIPE_TILT = PIPE_VOXEL, PIPE_TILT

-- Hotkeys: the six rows upstream DRAMATIC_SHAPE originally claimed with
-- number keys keep those numbers (this IS upstream now, not a fork sitting
-- beside it), so muscle memory from either mod still works for the rows
-- both had. The rows Terrarium added with no upstream equivalent keep their
-- original letters, since those never conflicted with 3/5/6/7/8/9 anyway.
local KEY_VOXEL  = "3"   -- VOXEL camera ladder: OFF/15/35/50/75/1ST/3RD (skips FULL)
local KEY_GRID   = "5"   -- V-GRID wireframe
local KEY_TILT   = "6"   -- T-SHIFT blur
local KEY_CURVE  = "7"   -- V-CURVE horizon
local KEY_BATTLE = "8"   -- 3D-BTL
local KEY_WILD   = "n"   -- WILD roam ladder (Terrarium-only row)
local KEY_MAP    = "p"   -- minimap ON/FULL/OFF (Terrarium-only row)
local KEY_HAZE   = "h"   -- V-HAZE aerial perspective (Terrarium-only row)
local KEY_SKYLINE = "k"  -- HORIZON far silhouettes (Terrarium-only row)
-- Fires one impact sheet at the player's feet and steps to the next on each
-- press -- the only way to SEE the pack without a fight, and the way to tell
-- "the row is off" apart from "the sheet did not decode".
local KEY_FX = "j"
V.KEYS = {
...
-- ------- merged in from upstream DRAMATIC_SHAPE (Advanced Voxel Mod)
--
-- The free-roam rig behind the VOXEL ladder's 1ST and 3RD rungs: FreeMove
-- swaps the grid walk for camera-relative movement on those two rungs only,
-- FirstPerson owns the eye-level camera and the mouse-look/capture
-- lifecycle, and CamControl is the shared zoom/lens front end used by the
-- third-person boom, a staged battle's camera and the engine's own survey
-- zoom alike. See lib/FirstPerson.lua for the full design note.
...
-- MERGE STATUS: these are all loaded and their per-frame ticks/installs are
-- wired below (search MERGE:), so the systems themselves run. Their OPTIONS
-- MENU rows (VR row, STADIUM rungs on 3D-BTL, HORDE's own settings, the
-- PLAYER MODEL picker screen) still need to be spliced into this file's
-- options-menu list by hand -- see the merge report -- so until then reach
-- them via their own hotkeys/hooks (VR toggle, the konami code for HORDE,
-- etc.) rather than the OPTIONS screen.
...
mod.content.render_pipelines:register(PIPE_VOXEL, {
  ...
  update = function(dt, level)
    ...
    -- MERGE: the first-person head, on the same tick: its blend in and out
    -- of the orbit, the mouse capture lifecycle, and the frame's stick-rate
    -- look. Unconditional like Voxel.update, because the blend has to keep
    -- easing OUT after the rung is left (level 6/7 -- see VoxelState.lua).
    -- MERGE: the atmosphere's own clock (shaft shimmer, drifting motes), on
    -- the same tick so the beams keep breathing through a dialog box.
    ForestAtmos.update(dt)
    FirstPerson.update(dt)
    -- MERGE: the horde minigame's always-running tick (konami code from the
    -- overworld); like the battle below it owns no pass of the frame and
    -- has to keep thinking under a warp's wipe and the GAME OVER card.
    Horde.update(dt)
    -- MERGE: LET'S GO's capture session, ahead of the battle's own update
    -- so the ball drawn a frame is the ball that frame computed. Guarded so
    -- a fault in the capture game costs the capture game, not the pipeline.
    do
    ...
    -- the camera.
    -- MERGE: the one-time build of the Pokemon Stadium battle models out of
    -- the player's own ROM (see lib/StadiumInstall.lua), and the player
    -- model load, on the same always-running tick as upstream had them.
    pcall(function()
      local okFollower, StadiumFollower = pcall(V.require, "StadiumFollower")
      if okFollower and StadiumFollower then StadiumFollower.checkDeferred() end
    end)
  ...
  invalidate = function()
    ...
    Water.dropGPU()
    MiniMap.invalidate()
    -- MERGE: the VR mirror and FBO ids of dead canvases, and the Poke
    -- Ball's own meshes/palette texture, on the same invalidation as
    -- everything else that owns GPU objects here.
    VR.invalidate()
    Pokeball.invalidate()
    -- MERGE: the forest haze/shaft meshes and shader sentinels
...
local SETTINGS = {
  ...
  -- ------- MERGE: upstream DRAMATIC_SHAPE row-s with no Terrarium
  -- equivalent. Anything upstream had that Terrarium already covers under a
  -- different module -- Water (Terrarium's is the geometry-swell version),
  -- shadow quality (Quality.shadowSetting here instead of upstream's
  -- Shadows.setting), BACK SPRITES (already OverworldBattle.backSetting
  -- above) -- is deliberately NOT re-added: registering the same concept
  -- twice under two different ModSetting objects would be two rows
  -- fighting over one behaviour, not two features.
  { Shiny.setting,
  ...
  { ViewBox.setting,
    ...
    .. "picture above about 63 degrees where the horizon comes into view.",
    full = true },
  { ForestAtmos.setting,
  ...
  { DrawDistance.setting,
    "How many adjacent maps to render: OFF (no limit), NEAR (0 "
    .. "neighbors) for best performance on low-end devices, MILD (2 "
    .. "neighbors), or FAR (4 neighbors).",
    full = true },
  { LetsGo.setting,
  ...
  { VR.smoothTurn,
    "Turns smoothly with the right stick instead of snapping 45 "
    .. "degrees, if you have your sea legs for it.",
...
-- MERGE: the free-roam rig behind VOXEL's 1ST/3RD rungs. FirstPerson claims
-- the look inputs the engine ignores; FreeMove wraps
-- OverworldState:handleInput, the one choke point every kind of programmed
-- walk (ledges, spinners, scripted steps) already goes through, so those
-- still play out on the grid with the free camera riding along; CamControl
-- is the shared zoom front end (wheel/Q-E/pinch/stick-click) for whichever
-- camera is in front of the player -- the third-person boom, a staged
-- battle's lens, or the engine's own survey zoom.
FirstPerson.install()
FreeMove.install()
CamControl.install()

-- MERGE: the konami code and everything it turns on (Horde Mode), then
-- LET'S GO capture mode -- in that order, matching upstream: LetsGo installs
-- last of the input seams so its aim-flick wraps sit outermost. Upstream
-- also exposes both through the pad's SELECT-button voxel-cycle hook, which
-- referenced an undefined local (`cycleVoxel`) in the source this was
-- merged from -- that looks like a pre-existing bug upstream, so it was not
-- carried over; the "3" key and the OPTIONS row still reach every rung.
Horde.install()
LetsGo.install()
...
mod.exports.version = "2.0.0-merge"
