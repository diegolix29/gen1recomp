...
function SpriteRenderer.new(spriteDef, seed)
  ...
  self.framesPerRow = positiveInteger(spriteDef.framesPerRow, 1)
  self.scale = numberOr(spriteDef.scale, 1.0)
  self.heightScale = numberOr(spriteDef.heightScale, self.scale)
  ...
  for f = 0, self.frameCount - 1 do
    -- By default frames are stacked vertically, matching the vanilla 16x96
    -- sheet.  Custom sheets may arrange frames in a grid with framesPerRow.
    local row = self.framesPerRow > 1 and math.floor(f / self.framesPerRow) or f
    local col = self.framesPerRow > 1 and (f % self.framesPerRow) or 0
    local x = col * self.frameWidth
    local y = row * self.frameHeight
    self.frames[f] = love.graphics.newQuad(
      x, y, self.frameWidth, self.frameHeight, iw, ih)
  end
  return self
end
...
function SpriteRenderer:getFrameGeometry(frame)
  ...
  local row = self.framesPerRow > 1
    and math.floor(frame / self.framesPerRow) or frame
  local col = self.framesPerRow > 1
    and (frame % self.framesPerRow) or 0
  return {
    frame = frame,
...
function SpriteRenderer:getScreenOrigin(px, py, camX, camY, scaleX, scaleY)
  scaleX = numberOr(scaleX, self.scale or 1)
  scaleY = numberOr(scaleY, self.heightScale or 1)
  local baseX = math.floor(px - camX) + WORLD_ANCHOR_X
  local baseY = math.floor(py - camY) + WORLD_ANCHOR_Y
  return math.floor(baseX - self.anchorX * scaleX),
         math.floor(baseY - self.anchorY * scaleY)
end
...
local function blitFrame(image, quad, x, y, flip, redraw, frameWidth,
  ...
  scaleX = numberOr(scaleX, 1)
  scaleY = numberOr(scaleY, 1)
  if flip then
    local drawX = x + frameWidth * scaleX
    love.graphics.draw(image, quad, drawX, y, 0, -scaleX, scaleY)
    if redraw then
      PaletteFX.markSpriteRedraw(image, quad, drawX, y, -scaleX)
    end
  else
    love.graphics.draw(image, quad, x, y, 0, scaleX, scaleY)
    if redraw then PaletteFX.markSpriteRedraw(image, quad, x, y, scaleX) end
  end
end
...
function SpriteRenderer:draw(px, py, camX, camY, facing, walkPhase, stepFlip,
  ...
  local x, y = self:getScreenOrigin(px, py, camX, camY, scaleX, scaleY)
  ...
  if topHalf and self.frameCount > 1 then
    ...
    if not self.halfFrames[frame] then
      ...
      local row = framesPerRow > 1
        and math.floor(frame / framesPerRow) or frame
      local col = framesPerRow > 1
        and (frame % framesPerRow) or 0
      local fx = col * self.frameWidth
      local fy = row * self.frameHeight
      self.halfFrames[frame] = love.graphics.newQuad(
        fx, fy, self.frameWidth, topHeight, iw, ih)
    end
  ...
  if self.def.trueColor then
    PaletteFX.markTrueColor(x, y, self.frameWidth * scaleX,
      drawHeight * scaleY)
  end
...
function SpriteRenderer:drawTile(path, x, y, flip)
  ...
  local scaleX = self.scale or 1.0
  local scaleY = self.heightScale or 1.0
  local tileWidth = self.frameWidth or DEFAULT_FRAME_WIDTH
  local tileHeight = math.floor((self.frameHeight or DEFAULT_FRAME_HEIGHT) / 2)
